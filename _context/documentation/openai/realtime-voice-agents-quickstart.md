...
The response will contain a "value" string at the top level, which starts with "ek_" prefix. You can use this ephemeral key to establish a WebRTC connection later on. Note that this key is only valid for a short period of time and will need to be regenerated.
